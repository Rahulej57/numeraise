...
export default function DateTimeCalculatorPage() {
  ...
  const relatedCalcs = getRelatedCalculators('date-time-calculator');
  return (
    <CalculatorLayout title="Date & Time Calculator" description="" icon={calculatorIcon ?? undefined}>
          ...
          <CardContent className="p-4 lg:p-6 space-y-6 flex-1 flex flex-col">
            ...
            <div className="mt-auto pt-6">
              <div className="bg-muted/10 border p-6 rounded-xl text-center">
                ...
                <div className="text-2xl font-bold text-primary min-h-[32px]">{calcDateData || '-'}</div>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <CalculatorContent>
        ...
        <p>
          A Date & Time Calculator is a utility tool designed to solve complex calendar math. Calculating the exact
          duration between two dates, or projecting a future date by adding a specific number of days, is notoriously
          difficult to do mentally because of leap years and varying month lengths.
        </p>
        ...
        <p>
          This mode answers the question: <em>"How long is it until..."</em> or <em>"How long ago was..."</em>. By
          selecting a Start Date and an End Date, the engine calculates the exact absolute difference in total days. It
          also breaks that duration down into human-readable format: Years, Months, and Days.
        </p>

        <h3>2. Add or Subtract from Date</h3>
        <p>
          This mode answers the question: <em>"What date will it be 90 days from now?"</em>. You select a Base Date,
          choose whether to add or subtract time, and input any combination of years, months, and days. The engine then
          perfectly calculates the exact calendar date (including the day of the week) that the jump lands on.
        </p>

        <h2>Common Uses for Calendar Math</h2>
        <ul>
          <li>
            <strong>Project Management:</strong> Calculate exact deadlines (e.g. "We need this done 45 days from
            Monday").
          </li>
          <li>
            <strong>Financial Grace Periods:</strong> Determine the exact day a 30-day grace period on a loan or credit
            card expires.
          </li>
          <li>
            <strong>Travel Visas:</strong> Ensure you don't overstay your welcome by calculating the exact 90th day of a
            tourist visa.
          </li>
          <li>
            <strong>Event Planning:</strong> Find out exactly how many days remain until a wedding, graduation, or
        ...
        faqs={[
          {
            ...
            answer:
              'Yes. Our internal calendar engine uses standard international time APIs which perfectly account for February 29th during leap years.',
          },
          {
            question: 'Can I enter a negative number of days?',
            answer:
              "There's no need! If you want to go backwards in time, simply use the 'Subtract From Date' toggle in the second calculator.",
          },
          {
            question: 'Why is the duration difference sometimes 1 day off from my math?',
            answer:
              'This is often due to whether or not you count the end date. Our calculator measures the exact duration *between* the days (e.g. Monday to Tuesday is exactly 1 day).',
          },
          {
            question: 'Does the calculator account for daylight saving time?',
            answer:
              'If the dates cross a daylight saving boundary, the exact hour duration will be affected. Standard day counting strictly calculates 24-hour periods.',
          },
          {
            question: 'What is a Business Day?',
            answer:
              'Business days generally exclude weekends (Saturday and Sunday) and official public holidays. This is crucial for banking and shipping estimates.',
          },
        ]}
      />

      <RelatedCalculators calculators={getRelatedCalculators('date-time-calculator')} />
      <StructuredData
        type="Calculator"
        data={{
          name: 'Date & Time Calculator',
          description:
            'Calculate the exact duration between two dates, or add/subtract days from a date to find a future deadline.',
        }}
      />
    </CalculatorLayout>
  );
