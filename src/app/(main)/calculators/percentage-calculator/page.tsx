...
export default function PercentageCalculatorPage() {
  ...
  return (
    <CalculatorLayout
      title="Percentage Calculator - Calculate Percentages and Changes"
      description=""
    ...
    >
          ...
          <CardContent className="p-4 lg:p-6 space-y-4">
            ...
            <div className="mt-6 pt-4 border-t bg-muted/10 -mx-4 lg:-mx-6 px-4 lg:px-6 pb-2 text-center">
              ...
              <div className="text-3xl font-bold text-primary h-10 flex items-center justify-center">
                {m1x && m1y
                  ? ((Number(m1x) / 100) * Number(m1y)).toLocaleString('en-US', { maximumFractionDigits: 4 })
                  : '-'}
              </div>
            </div>
          </CardContent>
        ...
        <Card className="bg-card shadow-sm border border-border">
          <CardContent className="p-4 lg:p-6 space-y-4">
            ...
            <div className="mt-6 pt-4 border-t bg-muted/10 -mx-4 lg:-mx-6 px-4 lg:px-6 pb-2 text-center">
              <div className="text-xs text-muted-foreground uppercase tracking-wider font-semibold mb-1">Result</div>
              <div className="text-3xl font-bold text-primary h-10 flex items-center justify-center">
                {m2x && m2y && Number(m2y) !== 0
                  ? ((Number(m2x) / Number(m2y)) * 100).toLocaleString('en-US', { maximumFractionDigits: 4 }) + '%'
                  : '-'}
              </div>
            </div>
          </CardContent>
            ...
            <div className="mt-6 pt-4 border-t bg-muted/10 -mx-4 lg:-mx-6 px-4 lg:px-6 pb-2 text-center">
              ...
              <div className="text-3xl font-bold h-10 flex items-center justify-center">
                {(() => {
                  ...
                  const relatedCalcs = getRelatedCalculators('percentage-calculator');
                  return (
                    <span className={isInc ? 'text-emerald-500' : isDec ? 'text-rose-500' : 'text-primary'}>
                      {isInc ? '+' : ''}
                      {change.toLocaleString('en-US', { maximumFractionDigits: 4 })}%
                    </span>
                  );
                })()}
              </div>
      ...
      <CalculatorContent>
        ...
        <p>
          A percentage calculator is a daily math utility designed to solve the three most common percentage problems
          encountered in business, retail, and everyday life. Whether you are trying to calculate a discount, figure out
          a profit margin, or determine tax rates, this tool handles the math instantly.
        </p>
        <h2>How Does It Work?</h2>
        <p>
          This tool provides three distinct calculation engines, each solving a different mathematical relationship:
        </p>

        <h3>1. What is X% of Y?</h3>
        <p>
          This mode is primarily used for finding discounts or tax amounts. For example, if you want to find out what a
          20% discount on a {currency.symbol}150 item is, the answer is {currency.symbol}30.
        </p>
        ...
        <div className="bg-muted p-4 rounded-lg my-4 text-center font-mono text-sm">
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            Example: (20 / 100) × {currency.symbol}150 = {currency.symbol}30
          </p>
        </div>

        <h3>2. X is what percent of Y?</h3>
        <p>
          This mode calculates proportions. If you scored 45 out of 60 on a test, you want to know what percentage that
          is.
        </p>
        ...
        <p>
          This mode is crucial for business and finance. It tells you exactly how much a value grew or shrank relative
          to its starting point. If a stock goes from {currency.symbol}50 to {currency.symbol}60, it represents a 20%
          Increase.
        </p>
        ...
        <div className="bg-muted p-4 rounded-lg my-4 text-center font-mono text-sm">
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            Example: (({currency.symbol}60 - {currency.symbol}50) / {currency.symbol}50) × 100 = 20%
          </p>
        </div>
        ...
        <ul>
          <li>
            <strong>Retail Discounts:</strong> Quickly figuring out exactly how much money a "30% Off" sale will
            actually save you.
          </li>
          <li>
            <strong>Sales Tax:</strong> Calculating the exact amount of tax you'll owe on a major purchase based on your
            local tax percentage.
          </li>
          <li>
            <strong>Business Growth:</strong> Calculating Month-over-Month (MoM) or Year-over-Year (YoY) revenue growth.
          </li>
        </ul>
      </CalculatorContent>
      <FAQAccordion
        faqs={[
          ...
          },
          {
            ...
            answer:
              'Subtract the old value from the new value, divide that difference by the old value, and then multiply by 100.',
          },
          {
            question: 'What is a basis point (BPS)?',
            answer:
              'In finance, a basis point is one-hundredth of a percentage point (0.01%). So, if an interest rate drops by 50 basis points, it means it dropped by 0.5%.',
          },
          {
            question: 'Are percentages reversible?',
            answer:
              'Yes! X% of Y is exactly equal to Y% of X. For example, 8% of 50 is the exact same as 50% of 8 (which is 4). This trick makes mental math much easier.',
          },
        ]}
      />
      ...
      <StructuredData
        ...
        data={{
          ...
          description:
            'Calculate standard percentages, proportions, and percentage changes (increases or decreases) instantly.',
        }}
      />
    </CalculatorLayout>
  );
