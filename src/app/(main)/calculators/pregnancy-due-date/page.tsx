...
export default function PregnancyDueDatePage() {
  ...
  const relatedCalcs = getRelatedCalculators('pregnancy-due-date');
  return (
    <CalculatorLayout title="Pregnancy Due Date Calculator" description="" icon={calculatorIcon ?? undefined}>
          ...
          <Card className="bg-card text-card-foreground shadow-lg overflow-hidden border border-border h-full flex flex-col min-h-[300px]">
            ...
            <CardContent className="p-4 lg:p-6 flex-1 flex flex-col justify-center text-center">
              {!pregnancyData ? (
                ...
                <div className="text-destructive font-medium">{pregnancyData.error}</div>
              ) : (
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4 divide-x">
                      <div>
                        ...
                        <div className="text-xl font-bold">
                          {pregnancyData.weeksPregnant}w {pregnancyData.daysPregnant}d
                        </div>
                      </div>
                      <div>
                        <div className="text-xs text-muted-foreground mb-1 uppercase tracking-wider font-semibold">
                        ...
                        <div className="text-xl font-bold">
                          ...
                          {pregnancyData.trimester === 1 ? 'st' : pregnancyData.trimester === 2 ? 'nd' : 'rd'}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-4">
                      <div className="w-full bg-muted rounded-full h-2.5 overflow-hidden">
                        <div
                          ...
                          style={{ width: `${pregnancyData.progress || 0}%` }}
                        ></div>
                      </div>
                  ...
                  <div className="text-sm text-muted-foreground bg-muted/10 p-3 rounded border">
                    ...
                    <span className="font-semibold text-foreground">{pregnancyData.conceptionDate}</span>
                  </div>
                </div>
              )}
            </CardContent>
      ...
      <CalculatorContent>
        ...
        <p>
          A Pregnancy Due Date Calculator is a medical forecasting tool used by obstetricians and expectant parents to
          estimate the Expected Date of Delivery (EDD). Since only about 4-5% of babies are born exactly on their due
          date, this date serves as an essential anchor point for tracking fetal development and scheduling medical
          checkups.
        </p>
        <h2>How Does the Due Date Calculator Work?</h2>
        <p>
          Our calculator primarily relies on Naegele's Rule, which is the standard medical formula used worldwide. By
          inputting the First Day of your Last Menstrual Period (LMP) and your average cycle length, the engine
          calculates the exact date you will hit the 40-week gestation mark.
        </p>

        <h3>The Naegele's Rule Mathematical Formula</h3>
        <p>
          Naegele's Rule assumes a standard 28-day menstrual cycle, with ovulation occurring on day 14. The formula is:
        </p>
        ...
        <p>
          If your average cycle is longer or shorter than 28 days, our calculator automatically adjusts the 280-day
          baseline to account for the delayed or early ovulation, giving you a mathematically personalized result.
        </p>

        <h2>Major Benefits of Calculating Your Due Date</h2>
        <ul>
          <li>
            <strong>Medical Tracking:</strong> Allows your doctor to accurately measure fetal growth against standard
            percentile charts for a specific gestational age.
          </li>
          <li>
            <strong>Maternity Leave Planning:</strong> Gives you a hard date to provide HR when organizing financial and
            professional arrangements.
          </li>
          <li>
            <strong>Trimester Milestones:</strong> Knowing your exact weeks and days pregnant helps you understand the
            physiological changes happening to your body week-to-week.
          </li>
        </ul>
      </CalculatorContent>
      <FAQAccordion
        faqs={[
          {
            ...
            answer:
              "Because it's usually impossible to know the exact day of conception. The first day of the last period is a highly visible, trackable biological event that doctors can use as a reliable starting anchor.",
          },
          {
            question: 'Is my due date guaranteed?',
            answer:
              "No. A 'due date' is simply an estimate of when you will be 40 weeks pregnant. A pregnancy is considered 'full term' anywhere between 37 and 42 weeks.",
          },
          {
            question: 'What if I know my exact conception date?',
            answer:
              'If you know your exact conception date (such as via IVF), simply add exactly 266 days to the conception date to find your true medical due date.',
          },
          {
            question: 'Why is a pregnancy calculated as 40 weeks?',
            answer:
              'Medical professionals date pregnancies from the first day of the last menstrual period (LMP), which is usually about two weeks before conception actually occurs.',
          },
          {
            question: 'Is the due date highly accurate?',
            answer:
              'No, only about 4% to 5% of babies are born on their exact calculated due date. It is simply an estimate. Delivery anytime between 37 and 42 weeks is considered normal.',
          },
        ]}
      />

      <RelatedCalculators calculators={getRelatedCalculators('pregnancy-due-date')} />
      <StructuredData
        type="Calculator"
        data={{
          name: 'Pregnancy Due Date Calculator',
          description:
            'Calculate your estimated due date, conception date, and current trimester based on your Last Menstrual Period.',
        }}
      />
    </CalculatorLayout>
  );
