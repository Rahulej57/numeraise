...
export default function TipCalculatorPage() {
  ...
  const relatedCalcs = getRelatedCalculators('tip-calculator');
  return (
    <CalculatorLayout title="Tip & Split Calculator" description="" icon={calculatorIcon ?? undefined}>
            ...
            <CardContent className="p-0 lg:p-6 space-y-6">
              <div className="space-y-2">
                ...
                <div className="relative">
                  <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none text-muted-foreground font-medium">
                    {currency.symbol}
                  </div>
                  <Input
                    id="bill"
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-bold">
                    {people} {Number(people) === 1 ? 'Person' : 'People'}
                  </span>
                </div>
        ...
        <div className="lg:col-span-6 space-y-2 lg:space-y-6">
          <Card className="bg-card text-card-foreground shadow-lg overflow-hidden border border-border h-full flex flex-col min-h-[300px]">
            <CardContent className="p-6 lg:p-8 flex-1 flex flex-col justify-center">
              {!tipData ? (
                <div className="text-muted-foreground text-center">Enter bill amount to see split.</div>
              ) : (
      ...
      <CalculatorContent>
        ...
        <p>
          A Tip & Split Calculator is an everyday dining utility designed to instantly calculate the exact gratuity on a
          restaurant bill and automatically divide the total cost evenly among a group of people. It eliminates the
          social awkwardness and mental math associated with splitting a check at the end of a meal.
        </p>
        <h2>How Does It Work?</h2>
        <p>
          The calculator requires three inputs: the total bill amount, your desired tip percentage, and the number of
          people splitting the bill. The engine first calculates the absolute tip amount. It then adds this to the
          original bill to find the 'Total Bill'. Finally, it divides that total by the number of people to give you the
          exact 'Per Person' contribution.
        </p>
        ...
        <ul>
          <li>
            <strong>Dining Out:</strong> Quickly figuring out a standard 18% or 20% tip at a restaurant without
            struggling with mental math after a few drinks.
          </li>
          <li>
            <strong>Group Events:</strong> Easily splitting the cost of a large pizza delivery, an Uber ride, or a
            shared Airbnb among friends.
          </li>
          <li>
            <strong>Service Industry:</strong> Calculating appropriate tips for hairdressers, massage therapists,
            delivery drivers, and tour guides.
          </li>
        </ul>
      </CalculatorContent>
      <FAQAccordion
        faqs={[
          {
            ...
            answer:
              'Etiquette experts generally agree that you should tip based on the pre-tax subtotal. However, it has become common practice in the US to simply tip on the final post-tax total for the sake of simplicity.',
          },
          {
            question: 'What is the standard tipping rate?',
            answer:
              'In the United States, 15% is considered minimum for average service, 18-20% is standard for good service, and 25%+ is for exceptional service. In many European and Asian countries, tipping is either not expected or a flat 10%.',
          },
          {
            question: 'How do I split the bill if someone ordered way more food?',
            answer:
              'This calculator is designed for an even split (where everyone pays the same amount). If one person ordered a $50 steak and everyone else ordered $10 salads, you should manually calculate their individual totals instead of splitting evenly.',
          },
          {
            question: 'Should I tip on the pre-tax or post-tax amount?',
            answer:
              'Proper etiquette dictates calculating the tip on the pre-tax subtotal. Tipping on the post-tax total means you are unnecessarily tipping on government taxes.',
          },
          {
            question: 'Is a service charge the same as a tip?',
            answer:
              'No. A mandatory service charge added to your bill goes entirely to the restaurant management, who may or may not distribute it to the staff. A tip goes directly to the server.',
          },
        ]}
      />

      <RelatedCalculators calculators={getRelatedCalculators('tip-calculator')} />
      <StructuredData
        type="Calculator"
        data={{
          name: 'Tip & Split Calculator',
          description:
            'Instantly calculate gratuity and divide a restaurant bill evenly among friends with our free tip calculator.',
        }}
      />
    </CalculatorLayout>
  );
