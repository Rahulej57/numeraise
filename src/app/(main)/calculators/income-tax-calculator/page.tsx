  ...
  const calculateTaxDetails = () => {
    ...
    if (currency.code === 'INR') {
      ...
      if (regime === 'new') {
        ...
        if (taxable > 600000) incomeTax += Math.min(taxable - 600000, 300000) * 0.1;
        if (taxable > 900000) incomeTax += Math.min(taxable - 900000, 300000) * 0.15;
        if (taxable > 1200000) incomeTax += Math.min(taxable - 1200000, 300000) * 0.2;
        if (taxable > 1500000) incomeTax += (taxable - 1500000) * 0.3;
        // rebate
        ...
        if (taxable > 500000) incomeTax += Math.min(taxable - 500000, 500000) * 0.2;
        if (taxable > 1000000) incomeTax += (taxable - 1000000) * 0.3;
        // rebate
        if (taxable <= 500000) incomeTax = 0;
      }
      incomeTax = Math.round(incomeTax * 1.04); // 4% Health & Education Cess
      return {
        totalTax: incomeTax / currency.rate,
        incomeTax: incomeTax / currency.rate,
        extraTaxes,
        standardDeduction: standardDeduction / currency.rate,
      };
    }
    ...
    if (currency.code === 'USD') {
      ...
      const taxable = Math.max(0, localSalary - localExemptions - standardDeduction);
      if (taxable > 0) incomeTax += Math.min(taxable, 11600) * 0.1;
      ...
      return {
        totalTax: (incomeTax + fica) / currency.rate,
        incomeTax: incomeTax / currency.rate,
        extraTaxes,
        standardDeduction: standardDeduction / currency.rate,
      };
    ...
    if (currency.code === 'GBP') {
      ...
      const taxable = Math.max(0, localSalary - localExemptions - standardDeduction);
      if (taxable > 0) incomeTax += Math.min(taxable, 37700) * 0.2;
      if (taxable > 37700) incomeTax += Math.min(taxable - 37700, 74870) * 0.4;
      ...
      return {
        totalTax: (incomeTax + ni) / currency.rate,
        incomeTax: incomeTax / currency.rate,
        extraTaxes,
        standardDeduction: standardDeduction / currency.rate,
      };
    ...
    if (currency.code === 'AUD') {
      ...
      return {
        totalTax: (incomeTax + medicare) / currency.rate,
        incomeTax: incomeTax / currency.rate,
        extraTaxes,
        standardDeduction: 0,
      };
    ...
    if (currency.code === 'SGD') {
      ...
      if (taxable > 280000) incomeTax += Math.min(taxable - 280000, 40000) * 0.2;
    ...
    const taxable = Math.max(0, localSalary - localExemptions);
    if (taxable > 15000) incomeTax += Math.min(taxable - 15000, 35000) * 0.2;
    if (taxable > 50000) incomeTax += (taxable - 50000) * 0.4;
...
Calculate your own: ${shareUrl}`;
  ...
  const relatedCalcs = getRelatedCalculators('income-tax-calculator');
  return (
    <div className="container mx-auto px-4 py-6 md:py-8 max-w-6xl">
        ...
        <div className="lg:col-span-6 space-y-6">
          <Card className="border-none shadow-none bg-muted/20">
            <CardContent className="p-5 md:p-6 space-y-6">
              ...
              <div
                className={`transition-opacity ${isIndian && regime === 'new' ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}
              >
                <SliderInput
                ...
                {isIndian && regime === 'new' && (
                  <p className="text-xs text-muted-foreground mt-2">
                    Deductions are not applicable in the New Tax Regime (except standard deduction of 50k).
                  </p>
                )}
              </div>
            </CardContent>
              ...
              <div className="mt-8 p-4 bg-muted rounded-lg border">
                ...
                {currency.code === 'EUR' && (
                  <p className="text-xs text-muted-foreground mt-2 py-2">
                    * Note: This is a generalized progressive estimate for Europe. Tax rates vary significantly across
                    member states (e.g. DE, FR, NL).
                  </p>
                )}
      ...
      <div className="mt-12">
        <CalculatorContent>
          ...
          <p>
            An Income Tax Calculator is a specialized financial tool designed to estimate your total annual tax
            liability based on your gross salary, location, and applicable deductions. Because tax laws utilize complex
            progressive bracket systems rather than a single flat percentage, calculating exactly how much of your
            paycheck you actually get to keep can be extremely difficult to do manually.
          </p>
          <h2>How Does the Tax Calculator Work?</h2>
          <p>
            Our calculator dynamically adapts to your selected currency and its corresponding geographical region.
            Whether you are using USD (IRS rules), GBP (HMRC rules), CAD (CRA rules), or INR, the engine automatically
            loads the exact progressive tax brackets and standard deductions for that specific tax authority.
          </p>

          <h3>Understanding Progressive Tax Brackets</h3>
          <p>
            The biggest misconception about taxes is that entering a higher tax bracket means all of your income is
            taxed at that higher rate. This is false. In a progressive system, your income is sliced into "chunks"
            (brackets), and only the money that falls within a specific chunk is taxed at that higher rate.
          </p>
          <ul>
            <li>
              <strong>Bracket 1 (e.g., $0 - $10,000):</strong> Might be taxed at 0%.
            </li>
            <li>
              <strong>Bracket 2 (e.g., $10,001 - $40,000):</strong> Might be taxed at 10%.
            </li>
            <li>
              <strong>Result:</strong> If you make $15,000, you only pay 10% on the $5,000 that fell into the second
              bracket. The first $10k is tax-free.
            </li>
          </ul>
          ...
          <ul>
            <li>
              <strong>Salary Negotiations:</strong> Knowing your true "In-Hand" salary helps you accurately negotiate
              job offers. A $10,000 raise might only translate to $6,500 in your pocket after jumping tax brackets.
            </li>
            <li>
              <strong>Tax Optimization:</strong> By playing with the "Deductions" slider, you can see exactly how much
              money you will save by maxing out your 401(k), IRA, or Section 80C investments.
            </li>
            <li>
              <strong>Budgeting:</strong> Allows you to accurately forecast your exact monthly take-home pay for
          ...
          faqs={[
            {
              ...
              answer:
                "Your 'Marginal' tax rate is the highest bracket your last dollar falls into (e.g., 24%). Your 'Effective' tax rate is the actual percentage of your total income you paid in taxes (e.g., 14.5%) after accounting for standard deductions and lower brackets. Our calculator provides your Effective Tax Rate.",
            },
            {
              question: "Why did you include a '0%' tax region for AED?",
              answer:
                'Because the United Arab Emirates (UAE) currently levies a 0% personal income tax on salaries. If you select AED, your gross salary is exactly equal to your net in-hand salary.',
            },
            {
              question: 'Does this calculator account for State/Provincial taxes?',
              answer:
                'No. Because state and local taxes (such as California State Tax or specific Canadian Provincial taxes) vary wildly depending on your exact zip code, this calculator provides an estimate strictly based on primary Federal/National tax brackets.',
            },
            {
              question: 'What is a standard deduction?',
              answer:
                'It is a flat deduction (currently ₹50,000 for salaried employees in India) that reduces your taxable income, regardless of your actual expenses.',
            },
            {
              question: 'Should I choose the Old or New Tax Regime?',
              answer:
                'The New Regime has lower tax rates but does not allow major deductions (like 80C or HRA). If your deductions exceed ₹3.75 Lakhs, the Old Regime is usually better. Otherwise, the New Regime saves more tax.',
            },
          ]}
        />

        <RelatedCalculators calculators={getRelatedCalculators('income-tax-calculator')} />
        <StructuredData
          type="Calculator"
          data={{
            name: 'Global Income Tax Calculator',
            description:
              'Calculate your estimated progressive income tax, effective tax rate, and in-hand take-home salary across multiple currencies.',
          }}
        />
      </div>
    </div>
