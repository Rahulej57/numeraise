...
export function DynamicCalculatorList({ calculators, variant = "list" }: DynamicCalculatorListProps) {
        ...
        {visibleCalculators.map((calc, idx) => (
          <Link href={calc.href} key={idx} className="group">
            <div className="h-full rounded-xl border border-border bg-muted/40 p-4 md:p-5 hover:bg-muted transition-all duration-300">
              <h3 className="font-semibold text-base md:text-lg group-hover:text-primary transition-colors flex items-center justify-between">
                {calc.name}
  ...
  return (
    <div className="flex flex-col divide-y divide-border">
      {visibleCalculators.map((calc) => (
        ...
        >
          <div className="mt-0.5 shrink-0 p-2 bg-muted rounded-lg group-hover:bg-primary/10 transition-colors">
